...
describe URIValidator, '#run' do
  ...
  it 'outputs an error for each invalid URI' do
    ...
    URIExtractor.stubs(:new).with(file_name).returns sites
    $stdout = io = StringIO.new
    URIValidator.new(file_name).run

    example_error = %r{ERROR in bad_json_file\.json: http://bad\.example\.org}
    tb_error = %r{ERROR in bad_json_file\.json: http://thoughtbot\.com/404}
    io.string.should =~ example_error
    io.string.should =~ tb_error
  end
